...
class BuildTarget:
    ...
    def __init__(self, anchors, na=3, bias=0.5, stride=[8, 16, 32], anchor_t=4, use_aux=False,
                 thread_num=4):
    ...
    def build_targets(self, p, targets, imgs, min_topk=10, g=0.5):
        ...
        nl = len(p)
        oo = time.time()
        data = [(batch_idx, p, targets, indices, anch, imgs[batch_idx].shape[1], min_topk)
